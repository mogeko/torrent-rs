...
use super::types::{PIPELINE_SIZE, PeerInfo, UT_PEX};
use super::{PeerManager, SwarmLoop};

/// Periodic PEX broadcast engine (BEP 11).
///
/// Extracted from [`SwarmLoop`] so the broadcast logic can be tested
/// independently.  Called from the `pex_tick` branch of the event loop.
pub(crate) struct PexManager {
    interval: Duration,
}
...
impl PexManager {
    ...
    pub async fn broadcast(
        &self, peers: &HashMap<SocketAddr, PeerInfo>, recently_dropped: &mut Vec<SocketAddr>,
        peer_mgr: &RwLock<PeerManager>,
    ) -> Result<(), Error> {
        let dropped_snapshot: Vec<SocketAddr> = std::mem::take(recently_dropped);
        ...
        for addr in addresses {
            ...
            if let Err(e) = self.send_one(&addr, &dropped, peers, peer_mgr).await {
    ...
    async fn send_one(
        &self, addr: &SocketAddr, dropped: &[SocketAddr], peers: &HashMap<SocketAddr, PeerInfo>,
        peer_mgr: &RwLock<PeerManager>,
    ) -> Result<(), Error> {
        let peer = match peers.get(addr) {
            Some(p) => p,
            None => return Ok(()),
        };
        ...
        let connected = peer_mgr.read().await.connection_addrs();
...
// ── SwarmLoop PEX methods (message handlers remain here) ──────────
...
impl SwarmLoop {
    ...
    pub(super) async fn send_pex_message(
        ...
        let interval = self.pex_manager.interval();
        if peer.last_pex_sent.is_some_and(|t| t.elapsed() < interval) {
            return Ok(());
        }
